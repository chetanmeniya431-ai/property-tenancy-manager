                                ...
                                @if($markingId === $req->id)
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <div class="flex gap-1">
                                            <button wire:click="saveResponse"
                                                    class="rounded bg-sky-600 px-2 py-1 text-xs font-medium text-white hover:bg-sky-700">Save</button>
                                            <button wire:click="cancelRespond"
                                                    class="rounded border border-slate-200 px-2 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50">Cancel</button>
                                        </div>
                                    </div>
                                @else
                                    <button wire:click="startRespond({{ $req->id }})"
                                            class="rounded border border-slate-200 px-2 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50 whitespace-nowrap">
                                        Mark responded
                                    </button>
                                @endif
                            @endif
